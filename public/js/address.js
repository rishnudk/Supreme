...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.loadAddressData = function(apiPath) {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                Swal.fire('Error', "Failed to load address: " + xhr.status + " " + xhr.statusText, 'error');
            }
        });
    };
    ...
    $("#updateAddressBtn").click(function(event) {
        ...
        if (!/^[a-zA-Z\s]{2,}$/.test(fullName)) {
            Swal.fire('Validation Error', 'Full Name must be at least 2 characters and contain only letters and spaces.', 'warning');
            return;
        }

        let phonePattern = /^\+?[0-9\s\-\(\)]{7,15}$/;
        if (!phonePattern.test(phone)) {
            Swal.fire('Validation Error', 'Phone must be 7-15 digits, optionally with +, spaces, or hyphens.', 'warning');
            return;
        }

        if (address.length < 5) {
            Swal.fire('Validation Error', 'Address must be at least 5 characters.', 'warning');
            return;
        }

        if (!/^[a-zA-Z\s]{2,}$/.test(city)) {
            Swal.fire('Validation Error', 'City must be at least 2 characters and contain only letters and spaces.', 'warning');
            return;
        }

        if (!/^[a-zA-Z\s]{2,}$/.test(state)) {
            Swal.fire('Validation Error', 'State must be at least 2 characters and contain only letters and spaces.', 'warning');
            return;
        }

        if (!/^[a-zA-Z\s]{2,}$/.test(country)) {
            Swal.fire('Validation Error', 'Country must be at least 2 characters and contain only letters and spaces.', 'warning');
            return;
        }
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                Swal.fire('Error', "Failed to update: " + xhr.status + " " + xhr.statusText, 'error');
            }
        });
    });
    ...
    $("#saveAddressBtn").click(function(event) {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                Swal.fire('Error', "Failed to save: " + xhr.status + " " + xhr.statusText, 'error');
            }
        });
    });
    ...
    window.deleteAddress = function() {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                Swal.fire('Error', "Failed to delete: " + xhr.status + " " + xhr.statusText, 'error');
            }
        });
    };
});